...
class K_Folder:
    ...
    def read_data(self):
        ...
        new_df = df[['image_name', 'f_3', 'f_4', 'f_8']]        # 新建一个df, 只取源csv中, image的名字和image的标签.
        for i in range(0, len(new_df)):
            img_name = new_df.loc[i][0]
            img_path = os.path.join(self.image_pth, img_name)
            img_label_1 = new_df.loc[i][1]
            img_label_2 = new_df.loc[i][2]
            img_label_3 = new_df.loc[i][3]
            dict_dataset[img_name] = [img_path, img_label_1, img_label_2, img_label_3]   # 后面在索引image的地址和标签时, 只需对应索引其list[0]和list[1]
